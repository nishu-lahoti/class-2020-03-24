...
options(scipen=999)
ggplot(train, aes(age, income, color = treatment)) + geom_point()
...
train_bootstrap <- train %>%
  ...
  group_by(replicate) %>%
  summarize(summary_average = mean(income))

ggplot(train_bootstrap, aes(x=summary_average)) + 
...
virtual_resamples <- tibble(replicate = 1:1000, sample = map(1:1000, ~rep_sample_n(train, size = 115, replace = TRUE, reps = 1)))
